...
def compute_dtw_alignment_scaled(file1, file2, selected_columns, framerate1, framerate2):
    ...
    perc1 = percentage_in_range(shifts_per_frame, best_shift-4, best_shift+4)
    perc2 = percentage_in_range(shifts_per_frame, avg_shift1-4, best_shift+4)

    return shifts_per_frame, avg_shift1, best_shift, perc1, perc2

if __name__ == "__main__":
    files_list1 = sorted(glob.glob(f"./seq7p_*_*_speed.csv"))
    files_list2 = sorted(glob.glob(f"./seqse_*_*_speed.csv"))
    framerate1 = 60
    framerate2 = 60
    ...
    for file1, file2 in zip(files_list1, files_list2):
        shifts_per_frame, shift1, best_shift1, perc1, perc2 = compute_dtw_alignment_scaled(file1, file2, selected_columns, framerate1, framerate2)
        ...
        print(f"{file1}-{file2} Estimated time shift: {best_shift1:.1f} {shift1:.1f} indices with {perc1:.2f} {perc2:.2f} confidence")
